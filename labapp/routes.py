...
#     return render_template('index.html',  pname='LOGIN', navmenu=navmenu)
# Обработка запроса к странице HOME
@app.route('/')
@app.route('/index')
def index():
    return render_template('index.html', pname='HOME', navmenu=navmenu)

#обработка запроса к странице about us
@app.route('/about')
def about():
    return render_template('about.html', pname='ABOUT US', navmenu=navmenu)
...
@app.route('/api/contactrequest/delete/<int:id>', methods=['DELETE'])
...
def register():
    ...
    if request.method == 'POST':
        ...
        return render_template('register.html',  navmenu=navmenu)
...
# Обработка AJAX-запроса из формы со страницы /contact
...
@login_required
def contact():
    return render_template('contact.html', pname='CONTACT', navmenu=navmenu)
